...
1. `manifest.json`에 다음 내용을 직접 추가:
   ```json
...
### 하위 호환 API (BlankReadAssets)

- **Read** - `byte[]` 배열로 읽기 (실패 시 `null` 반환):
...
### 전체 API (BetterStreamingAssets)

```csharp
// 첫 사용 전 초기화 (메인 스레드에서 호출 필요)
